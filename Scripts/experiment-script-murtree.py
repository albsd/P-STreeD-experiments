...
program_infos = [
    {
        "path": "murtree/LinuxRelease",
        "program": "./MurTree"
    },
]

max_depth_num_nodes_combinations = [
    # {"max-depth": "4", "max-num-nodes": "15"},
    # {"max-depth": "5", "max-num-nodes": "24"},
    {"max-depth": "5", "max-num-nodes": "31"},
]
...
base_csv_filename = "run_times_murtree_d5n31"
csv_extension = "csv"
    ...
    for data_file in data_files:
        for combo in max_depth_num_nodes_combinations:
            command = [
                prog_info['program'],
                "-file", data_file,
                "-max-depth", combo["max-depth"],
                "-max-num-nodes", combo["max-num-nodes"],
                "-time", str(timeout)
            ]
            ...
            for i in range(n):
                ...
                with open(csv_file, mode='a', newline='') as file:
                    ...
                    writer.writerow({
                        ...
                        "task": "accuracy",
                        "file": data_file,
                        "max_depth": combo["max-depth"],
                        "max_num_nodes": combo["max-num-nodes"],
                        "status": status,
                ...
                print(f"Completed run {i + 1} for program {prog_info['program']} with file {data_file}, max_depth {combo['max-depth']}, max_num_nodes {combo['max-num-nodes']}")
